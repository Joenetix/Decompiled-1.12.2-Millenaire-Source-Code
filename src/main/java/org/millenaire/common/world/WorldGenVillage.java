...
public class WorldGenVillage implements IWorldGenerator {
   ...
   private static Field FIELD_BIOME_NAME = ReflectionHelper.findField(Biome.class, "biomeName", "field_76791_y");
   private static HashSet<Integer> chunkCoordsTried = new HashSet<>();

   public static boolean generateBedrockLoneBuilding(
         Point p, World world, VillageType village, Random random, int minDistance, int maxDistance,
         EntityPlayer player) throws MillLog.MillenaireException {
      if (world.isRemote) {
         return false;
         ...
         MillLog.printException(
               new MillLog.MillenaireException("Tried to create a bedrock lone building without a centre."));
         return false;
      } else {
   ...
   private static boolean isWithinSpawnRadiusProtection(World world, VillageType villageType, Point villagePos) {
      if (MillConfigValues.spawnProtectionRadius == 0) {
         ...
         if (villagePos.horizontalDistanceTo(world.getSpawnPoint()) < MillConfigValues.spawnProtectionRadius
               + villageRadius) {
            if (MillConfigValues.LogWorldGeneration >= 3) {
               MillLog.debug(
                     ...
                     "Blocking spawn at "
                           + villagePos
                           + ". Distance to spawn: "
                           + villagePos.horizontalDistanceTo(world.getSpawnPoint())
                           + ", min acceptable : "
   ...
   public void generate(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
      ...
      if (world.provider.getDimension() == 0) {
         ...
         if (MillConfigValues.villageSpawnCompletionMaxPercentage > 0
               && distanceToSpawn > MillConfigValues.villageSpawnCompletionMinDistance) {
            if (distanceToSpawn > MillConfigValues.villageSpawnCompletionMaxDistance) {
               ...
               completionRatio = (float) (MillConfigValues.villageSpawnCompletionMaxPercentage
                     * ((distanceToSpawn - MillConfigValues.villageSpawnCompletionMinDistance)
         ...
         try {
            this.generateVillageAtPoint(
                  world, random, chunkX * 16, 0, chunkZ * 16, null, true, false, true, Integer.MAX_VALUE, null, null,
                  null, completionRatio);
         } catch (Exception var13) {
            MillLog.printException(
                  "Exception when attempting to generate village in " + world + " (dimension: "
   ...
   private boolean generateCustomVillage(Point p, World world, VillageType villageType, EntityPlayer player,
         Random random) throws MillLog.MillenaireException {
      ...
      mw.registerVillageLocation(
            world, townHall.getPos(), townHall.getVillageQualifiedName(), townHall.villageType, townHall.culture, true,
            player.getName());
   ...
   private void generateHamlet(World world, VillageType hamlet, Point centralVillage, String name, Random random,
      ...
      for (int minRadius = 250; !generated && minRadius < 350; minRadius += 50) {
         ...
         for (int attempts = 0; !generated && attempts < 36; attempts++) {
            ...
            if (MillConfigValues.LogWorldGeneration >= 1) {
               MillLog.major(this, "Trying to generate a hamlet " + hamlet + " around: " + (centralVillage.getiX() + dx)
   ...
   private boolean generateVillage(
         ...
         boolean alwaysSpawn) throws MillLog.MillenaireException {
      ...
      MillWorldData mw = Mill.getMillWorld(world);
      targetPos = new Point(targetPos.x, WorldUtilities.findTopSoilBlock(world, targetPos.getiX(), targetPos.getiZ()),
            targetPos.z);

      for (int x = targetPos.getChunkX() - villageType.radius / 16 - 1; x <= targetPos.getChunkX()
            + villageType.radius / 16; x++) {
         for (int z = targetPos.getChunkZ() - villageType.radius / 16 - 1; z <= targetPos.getChunkZ()
               + villageType.radius / 16; z++) {
      ...
      if (!alwaysSpawn && !this.isUsableArea(winfo, targetPos, villageType.radius)) {
         ...
         if (location == null) {
            ...
            if (isWithinSpawnRadiusProtection(world, villageType, thPos)) {
               ...
               MillLog.minor(
                     ...
                     "Distance to spawn of "
                           + location.pos.horizontalDistanceTo(world.getSpawnPoint())
                           + " is sufficient. Pos: "
                           + thPos
                           + ", spawn: "
                           + world.getSpawnPoint());
            ...
            if (!alwaysSpawn) {
               ...
               if (villageType.lonebuilding) {
                  if (villageType.isKeyLoneBuildingForGeneration(closestPlayer)) {
                     minDistanceWithVillages = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenVillagesAndLoneBuildings) / 2;
                     minDistanceWithLoneBuildings = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenLoneBuildings) / 2;
                  } else {
                     minDistanceWithVillages = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenVillagesAndLoneBuildings);
                     minDistanceWithLoneBuildings = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenLoneBuildings);
                  }
               } else {
               ...
               for (Point thp : mw.villagesList.pos) {
                  if (thPos.distanceTo(thp) < minDistanceWithVillages) {
                     if (MillConfigValues.LogWorldGeneration >= 1) {
                        MillLog.major(this, "Found a nearby village with final position. TargetPos / ThPos distance: "
                              + targetPos.directionTo(thPos));
                     }
               ...
               for (Point thpx : mw.loneBuildingsList.pos) {
                  if (thPos.distanceTo(thpx) < minDistanceWithLoneBuildings) {
                     if (MillConfigValues.LogWorldGeneration >= 1) {
                        MillLog.major(this, "Found a nearby lone building final position. TargetPos / ThPos distance: "
                              + targetPos.directionTo(thPos));
                     }
            ...
            if (MillConfigValues.LogWorldGeneration >= 2) {
               MillLog.minor(this,
                     "Place found for TownHall (village type: " + villageType.key + "). Checking for the rest.");
            }
            ...
            for (BuildingPlanSet planSet : villageType.startBuildings) {
               location = planSet.getRandomStartingPlan().findBuildingLocation(winfo, regionMapper, thPos,
            ...
            if (!couldBuildKeyBuildings) {
               ...
               if (MillConfigValues.LogWorldGeneration >= 1) {
                  for (BuildingLocation bl : plannedBuildings) {
                     MillLog.major(this, "Building " + bl.planKey + ": " + bl.minx + "/" + bl.minz + " to " + bl.maxx
                           + "/" + bl.maxz);
                  }
               }
               ...
               for (int i = 1; i < plannedBuildings.size(); i++) {
                  ...
                  if (bl.level == -1) {
                     if ((!(completionRatio > 0.0F) || !(Math.random() <= completionRatio))
                           && !(completionRatio > 0.33)) {
                        BuildingProject project = new BuildingProject(planSetx);
                        project.location = bl;
                        if (planSetx.getFirstStartingPlan().isWallSegment) {
                           if (!townHallEntity.buildingProjects
                                 .containsKey(BuildingProject.EnumProjects.WALLBUILDING)) {
                              townHallEntity.buildingProjects.put(BuildingProject.EnumProjects.WALLBUILDING,
                  ...
                  if (bl.level >= 0) {
                     lbps = planSetx.buildLocation(mw, villageType, bl, true, false, townHallEntity, false, false,
                           player);
                     if (MillConfigValues.LogWorldGeneration >= 1) {
               ...
               if (completionRatio > 0.0F && !villageType.playerControlled) {
                  ...
                  for (BuildingProject project : townHallEntity.getFlatProjectList()) {
                     if (project.location != null) {
                        nbRushedBuildingsTarget = project.planSet.plans.get(project.location.getVariation()).length
                  ...
                  while (nbBuildingRushed < nbRushedBuildingsTarget || completionRatio >= 1.0F) {
                     ...
                     if (nbRushed == 0) {
                        MillLog.temp(
                              townHallEntity, "Finished rushing at " + nbBuildingRushed + " on "
                                    + nbRushedBuildingsTarget + " as no more rushing was possible.");
                        break;
                     }
                  ...
                  MillLog.temp(townHallEntity,
                        "Finished rushing at " + nbBuildingRushed + " on " + nbRushedBuildingsTarget + ".");
               }
               ...
               if (villageType.lonebuilding) {
                  ...
                  if (villageType.playerControlled) {
                     townHallEntity
                           .storeItemStack(ItemParchment.createParchmentForVillage(townHallEntity.getTownHall()));
                  }
               }
               ...
               for (String key : villageType.hamlets) {
                  ...
                  if (hamlet != null) {
                     ...
                     this.generateHamlet(world, hamlet, townHallEntity.getPos(),
                           townHallEntity.getVillageNameWithoutQualifier(), random, completionRatio);
                  }
               }
      ...
      if (world.isRemote) {
         ...
         if (mw == null) {
            ...
            if (Mill.loadingComplete
                  && (generateVillages || MillConfigValues.generateLoneBuildings || alwaysGenerate)) {
               ...
               if (generatingPlayer == null) {
                  closestPlayer = world.getClosestPlayer(x, 64.0, z, 200.0, false);
               }
               ...
               try {
                  ...
                  if (checkForUnloaded) {
                     ...
                     p = this.generateVillageAtPoint_checkForUnloaded(world, x, y, z, generatingPlayer, p,
                           villageRadius);
                     if (p == null) {
                        return false;
                     }
                  ...
                  if (generateVillages || alwaysGenerate) {
                     boolean canAttemptVillage = this.generateVillageAtPoint_canAttemptVillage(world, generatingPlayer,
                           minDistance, mw, p, startTime);
                     if (canAttemptVillage) {
                        VillageType villageType;
                        if (specificVillageType == null) {
                           villageType = this.generateVillageAtPoint_findVillageType(world, p.getiX(), p.getiZ(), mw,
                                 closestPlayer);
                        } else {
                           villageType = specificVillageType;
                        }
                  ...
                  if (generatingPlayer == null && MillConfigValues.generateLoneBuildings
                        && specificVillageType == null) {
                     boolean keyLoneBuildingsOnly = false;
                     int minDistanceWithVillages = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenVillagesAndLoneBuildings);
                     int minDistanceWithLoneBuildings = Math.min(minDistance,
                           MillConfigValues.minDistanceBetweenLoneBuildings);
                     ...
                     for (Point thp : mw.villagesList.pos) {
                        if (p.distanceTo(thp) < minDistanceWithVillages / 2) {
                           if (MillConfigValues.LogWorldGeneration >= 3) {
                              MillLog.debug(this,
                                    "Time taken for finding near villages: " + (System.nanoTime() - startTime));
                           }
                     ...
                     if (MillConfigValues.LogWorldGeneration >= 3) {
                        MillLog.debug(this,
                              "Time taken for finding near villages (not found): " + (System.nanoTime() - startTime));
                     }
                     ...
                     for (Culture c : Culture.ListCultures) {
                        for (VillageType vt : c.listLoneBuildingTypes) {
                           if (vt.isValidForGeneration(mw, closestPlayer, nbLoneBuildings, new Point(x, 60.0, z),
                                 biomeName, keyLoneBuildingsOnly)) {
                     ...
                     if (acceptableLoneBuildingsType.size() == 0) {
                        ...
                        VillageType loneBuilding = (VillageType) MillCommonUtilities
                              .getWeightedChoice(acceptableLoneBuildingsType, closestPlayer);
                        if (MillConfigValues.LogWorldGeneration >= 2) {
   ...
   private boolean generateVillageAtPoint_canAttemptVillage(
         World world, EntityPlayer generatingPlayer, int minDistance, MillWorldData mw, Point p, long startTime) {
   ...
   private Point generateVillageAtPoint_checkForUnloaded(World world, int x, int y, int z,
         EntityPlayer generatingPlayer, Point p, int villageRadius) {
      boolean areaLoaded = false;
      int chunkRadius = villageRadius / 16 + 2;
      if (!WorldUtilities.checkChunksGenerated(world, x - 16 * chunkRadius, z - 16 * chunkRadius, x + 16 * chunkRadius,
            z + 16 * chunkRadius)) {
         for (int i = -8; i <= 8 && !areaLoaded; i++) {
            for (int j = -8; j <= 8 && !areaLoaded; j++) {
               ...
               if (!chunkCoordsTried.contains(this.computeChunkCoordsHash(tx, tz))
                     && WorldUtilities.checkChunksGenerated(world, tx - 16 * chunkRadius, tz - 16 * chunkRadius,
                           tx + 16 * chunkRadius, tz + 16 * chunkRadius)) {
                  areaLoaded = true;
   ...
   private VillageType generateVillageAtPoint_findVillageType(World world, int x, int z, MillWorldData mw,
         EntityPlayer closestPlayer) throws IllegalArgumentException, IllegalAccessException {
      ...
      for (Culture c : Culture.ListCultures) {
         for (VillageType vt : c.listVillageTypes) {
            if (vt.isValidForGeneration(Mill.getMillWorld(world), closestPlayer, nbVillages, new Point(x, 60.0, z),
                  biomeName, false)) {
   ...
   private String getBiomeNameAtPos(World world, int x, int z) throws IllegalAccessException {
      return ((String) FIELD_BIOME_NAME.get(world.getBiome(new BlockPos(x, 0, z)))).toLowerCase();
   }
   ...
   private boolean isInValidBiomes(World world, VillageType villageType, Point p) {
      ...
      if (MillConfigValues.LogWorldGeneration >= 2) {
         MillLog.minor(villageType, "Biome validity around " + p + ": " + validPerc + ", required: "
               + villageType.getMinimumBiomeValidity());
      }
