...
public class VillageWallGenerator {
        ...
        for (WallSegment segment : wallSegments) {
            ...
            if (segment.previousSegment != null && segment.nextSegment != null) {
                if (segment.previousSegment.location.pos.y < y && segment.nextSegment.location.pos.y < y) {
                    segment.setYLevel((int) Math.max(segment.previousSegment.location.pos.y,
                            segment.nextSegment.location.pos.y));
                    y = segment.location.pos.getiY();
                } else if (segment.previousSegment.location.pos.y > y && segment.nextSegment.location.pos.y > y) {
                    segment.setYLevel((int) Math.min(segment.previousSegment.location.pos.y,
                            segment.nextSegment.location.pos.y));
                    y = segment.location.pos.getiY();
                }
            }
            ...
            if (segment.sloppable) {
                if (segment.previousSegment != null && !segment.previousSegment.sloppable
                        && segment.previousSegment.location.pos.y < y) {
                    segment.setYLevel(segment.previousSegment.location.pos.getiY());
                    y = segment.location.pos.getiY();
                } else if (segment.nextSegment != null && !segment.nextSegment.sloppable
                        && segment.nextSegment.location.pos.y < y) {
                ...
                if (slopeKey != null) {
                    // Assuming createLocationForAlternateBuilding exists or implementing it
                    // BuildingLocation doesn't have createLocationForAlternateBuilding in Source
                    // currently.
                    // I will implement a simpler version here or add it to BuildingLocation.
                    // For now, assume it's missing and work around or TODO.
                    // Actually, I should probably add it to BuildingLocation if I want "exact
                    // match".

                    // Workaround: Clone and change key
                    BuildingLocation newLoc = segment.location.clone();
                    if (newLoc != null) {
                        newLoc.planKey = slopeKey;
                        segment.location = newLoc;
                        MillLog.temp(null, "Placed slope " + segment.location.planKey + ", yTowardsPrevious: "
                                + segment.yTowardsPrevious + ", yTowardsNext:" + segment.yTowardsNext);
                    }
                }
            }
        }
    ...
    private void buildNextElements(RegionMapper regionMapper, Point centre, VillageType villageType, WallType wallType,
            List<WallSegment> locationsBackward, List<WallSegment> locationsForward, VillageMapInfo winfo,
            int wallRadius, WallSide side, int pos, BuildingPlanSet planSet, boolean spawn, boolean buildNegative)
        ...
        if (side.xMultiplier != 0) {
            ...
            WallSegment segment = this.computeWallElementLocation(regionMapper, centre, villageType, wallType, winfo,
                    planSet, p, side.buildingOrientation, spawn);
            if (segment != null) {
            ...
            if (buildNegative) {
                ...
                segment = this.computeWallElementLocation(regionMapper, centre, villageType, wallType, winfo, planSet,
                        p, side.buildingOrientation, spawn);
                if (segment != null) {
            ...
            WallSegment segmentx = this.computeWallElementLocation(regionMapper, centre, villageType, wallType, winfo,
                    planSet, px, side.buildingOrientation, spawn);
            if (segmentx != null) {
            ...
            if (buildNegative) {
                ...
                segmentx = this.computeWallElementLocation(regionMapper, centre, villageType, wallType, winfo, planSet,
                        px, side.buildingOrientation, spawn);
                if (segmentx != null) {
    ...
    private void capWalls(Point villageCentre, List<WallSegment> wallSegments, BuildingPlan wallPlan,
            BuildingPlan capPlanRight, BuildingPlan capPlanLeft, BuildingPlan capPlanBoth) {
        for (WallSegment segment : wallSegments) {
            if (segment.location.getPlan() == wallPlan) {
                ...
                if (target != null && target != wallPlan) {
                    // BuildingLocation(BuildingPlan plan, Point ppos, int porientation)
                    BuildingLocation newLocation = new BuildingLocation(target, segment.location.pos,
                            segment.location.orientation);
    ...
    private int computeAverageYLevel(BuildingPlan plan, int orientation, Point pos) {
        ...
        corners.add(pos.getRelative(-orientatedLength / 2, 0.0, orientatedWidth / 2));
        corners.add(pos.getRelative(-orientatedLength / 2, 0.0, -orientatedWidth / 2));
        int ySurfaceLevel = 0; // Fix: was 2 in decompiled?? Probably biased average.
    ...
    public List<BuildingLocation> computeWallBuildingLocations(VillageType villageType, WallType wallType,
            int maxWallRadius, RegionMapper regionMapper, Point centre, VillageMapInfo winfo)
            throws MillLog.MillenaireException {
        ...
        // BuildingPlan gatewayPlan = gatewayPlanSet != null ?
        // gatewayPlanSet.getFirstStartingPlan() : null; // Unused variable warning in
        // eclipse?
        // Used below
        ...
        for (WallSide side : sides) {
            ...
            for (int var42 = 0; pos < wallRadiusLimit; var42++) {
                ...
                if (currentPlanSet != null) {
                    this.buildNextElements(regionMapper, centre, villageType, wallType, segmentsBackward,
                            segmentsForward, winfo, wallRadius, side, pos, currentPlanSet, spawn, true);
                }
            ...
            if (wallPlanSet != null) {
                this.buildNextElements(regionMapper, centre, villageType, wallType, segmentsBackward, segmentsForward,
                        winfo, wallRadius, side, pos, wallPlanSet, wallType.villageWallSpawn, true);
            }

            pos += wallLength;
            if (cornerPlanSet != null) {
                this.buildNextElements(regionMapper, centre, villageType, wallType, segmentsBackward, segmentsForward,
                        winfo, wallRadius, side, pos, cornerPlanSet, wallType.villageWallCornerSpawn, false);
            }

            Collections.reverse(segmentsBackward);
            wallSegments.addAll(segmentsBackward);
            WallSegment gatewaySegment = this.computeWallElementLocation(regionMapper, centre, villageType, wallType,
                    winfo, gatewayPlanSet, gatewayPos, side.buildingOrientation, wallType.villageWallGatewaySpawn);
    ...
    private void computeWallConnections(List<WallSegment> wallSegments) {
        ...
        for (int i = 0; i < wallSegments.size(); i++) {
            WallSegment previousSegment = i == 0 ? wallSegments.get(wallSegments.size() - 1)
                    : wallSegments.get(i - 1);
            WallSegment currentSegment = wallSegments.get(i);
            boolean connectedToPrevious = previousSegment.location.pos
                    .horizontalDistanceTo(currentSegment.location.pos) < (previousSegment.location
                            .getPlan().length + currentSegment.location.getPlan().length) / 2 + 4;
    ...
    private WallSegment computeWallElementLocation(RegionMapper regionMapper, Point centre, VillageType villageType,
            WallType wallType, VillageMapInfo winfo, BuildingPlanSet planSet, Point pos, int orientation, boolean spawn)
            throws MillLog.MillenaireException {
        ...
        if (y > centre.y + wallType.maxYDelta) {
            ...
            return new WallSegment(location);
        }
    }
    ...
    private boolean isReachablePos(RegionMapper regionMapper, VillageMapInfo winfo, Point pos) {
        ...
        return relativeX >= 0 && relativeX < regionMapper.regions.length
                && relativeZ >= 0 && relativeZ < regionMapper.regions[0].length
