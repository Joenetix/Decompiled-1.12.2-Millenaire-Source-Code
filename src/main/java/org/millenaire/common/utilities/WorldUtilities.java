...
public class WorldUtilities {
   ...
   public static int findSurfaceBlock(World world, int x, int z) {
      ...
      while (
         pos.getY() > -1
            ...
            && !(getBlock(world, x, pos.getY(), z) instanceof BlockLiquid)
      ) {
   ...
   public static IBlockState getBlockStateValidGround(IBlockState currentBlockState, boolean surface) {
      ...
      if (b == Blocks.BEDROCK) {
         ...
         return b == Blocks.HARDENED_CLAY ? currentBlockState : null;
      }
   }
   ...
   public static List<Entity> getEntitiesWithinAABB(World world, Class type, Point p, int hradius, int vradius) {
      AxisAlignedBB area = new AxisAlignedBB(p.x, p.y, p.z, p.x + 1.0, p.y + 1.0, p.z + 1.0)
         .expand(hradius, vradius, hradius)
         .expand(-hradius, -vradius, -hradius);
   ...
   public static void playSoundBlockBreaking(World world, Point p, Block b, float volume) {
      if (b != null && b.getSoundType() != null) {
         playSound(
            world, p, b.getSoundType().getBreakSound(), SoundCategory.BLOCKS, b.getSoundType().getVolume() * volume, b.getSoundType().getPitch()
         );
      }
   }
   ...
   public static void playSoundBlockPlaced(World world, Point p, Block b, float volume) {
      if (b != null && b.getSoundType() != null) {
         playSound(
            world, p, b.getSoundType().getPlaceSound(), SoundCategory.BLOCKS, b.getSoundType().getVolume() * volume, b.getSoundType().getPitch()
         );
      }
   }
   ...
   public static boolean setBlock(World world, Point p, Block block, boolean notify, boolean playSound) {
      if (p.x < -3.2E7 || p.z < -3.2E7 || p.x >= 3.2E7 || p.z > 3.2E7) {
         ...
         if (playSound && block == Blocks.AIR) {
            ...
            if (oldBlock != null && oldBlock.getSoundType() != null) {
         ...
         if (playSound && block != Blocks.AIR && block.getSoundType() != null) {
   ...
   public static void spawnExp(World world, Point p, int nb) {
      if (!world.isRemote) {
         ...
         while (j > 0) {
            ...
            j -= l;
            world.spawnEntity(new EntityXPOrb(world, p.x + 0.5, p.y + 5.0, p.z + 0.5, l));
         }
      }
   }
      ...
      for (int i = 0; i < nb; i++) {
         ...
         if (entityliving != null) {
            ...
            for (int j = 0; j < 20 && !spawned; j++) {
               double ex = p.x + (world.rand.nextDouble() * 2.0 - 1.0) * radius;
               double ey = p.y + world.rand.nextInt(3) - 1.0;
               double ez = p.z + (world.rand.nextDouble() * 2.0 - 1.0) * radius;
