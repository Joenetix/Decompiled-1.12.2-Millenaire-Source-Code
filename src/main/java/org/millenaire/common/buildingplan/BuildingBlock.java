...
public class BuildingBlock {
   ...
   public boolean build(World world, Building townHall, boolean worldGeneration, boolean wandimport) {
      ...
      try {
         ...
         if (this.special == 0) {
            ...
            IBlockState furnaceBS = Blocks.FURNACE.getDefaultState().withProperty(BlockFurnace.FACING, facing);
            world.setBlockState(this.p.getBlockPos(), furnaceBS);
            blockSet = true;
         } else if (this.special == CHESTGUESS) {
            EnumFacing facing = this.guessChestFurnaceFacing(world, this.p);
            IBlockState chestBS = MillBlocks.LOCKED_CHEST.getDefaultState().withProperty(BlockChest.FACING, facing);
            ...
            if (this.blockState.getValue(BlockDoor.HALF) == EnumDoorHalf.LOWER) {
               IBlockState bs = this.p.getBlockActualState(world).withProperty(BlockDoor.HALF, EnumDoorHalf.UPPER);
               bs = bs.withProperty(BlockDoor.HINGE, EnumHingePosition.RIGHT);
   ...
   private boolean buildClearGround(World world, boolean worldGeneration, boolean wandimport, boolean notifyBlocks, boolean playSound) {
      ...
      if ((!wandimport || existingBlock != Blocks.STANDING_SIGN && existingBlock != MillBlocks.IMPORT_TABLE)
         ...
         if (this.special == CLEARGROUNDBORDER && !(existingBlock instanceof BlockLeaves) && existingBlock != Blocks.AIR) {
            if (this.p.getEast().getBlock(world) instanceof BlockLiquid
               || this.p.getWest().getBlock(world) instanceof BlockLiquid
               || this.p.getNorth().getBlock(world) instanceof BlockLiquid
               || this.p.getSouth().getBlock(world) instanceof BlockLiquid) {
            ...
            && (this.special != CLEARGROUNDOUTSIDEBUILDING && this.special != CLEARGROUNDBORDER || !(existingBlock instanceof BlockLeaves))) {
            targetBlockState = Blocks.AIR.getDefaultState();
            shouldSetBlock = true;
         }
      }

      IBlockState blockStateBelowx = WorldUtilities.getBlockState(world, this.p.getBelow());
      ...
      if (worldGeneration && targetBelowBlockState == Blocks.DIRT.getDefaultState()) {
         ...
         && (targetBlockState != Blocks.DIRT.getDefaultState() || blockStateBelowx.getBlock() != Blocks.GRASS)) {
         shouldSetBlock = true;
      }
   ...
   private boolean buildClearTree(World world, boolean worldGeneration, boolean notifyBlocks, boolean playSound) {
      ...
      if (!(block instanceof BlockLog)) {
         ...
         if (worldGeneration && targetBlockState != null && targetBlockState.getBlock() == Blocks.DIRT) {
            WorldUtilities.setBlock(world, this.p.getBelow(), Blocks.GRASS, notifyBlocks, playSound);
         } else if (targetBlockState != null && (targetBlockState != Blocks.DIRT.getDefaultState() || block != Blocks.GRASS)) {
   ...
   private boolean buildNormalBlock(World world, Building townHall, boolean wandimport, boolean notifyBlocks, boolean playSound) {
      ...
      if (this.block instanceof BlockDoor) {
         ...
         EnumFacing facing = (EnumFacing)this.blockState.getValue(BlockHorizontal.FACING);
      ...
      if (!wandimport || this.block != Blocks.AIR || WorldUtilities.getBlock(world, this.p) != Blocks.STANDING_SIGN) {
         ...
         if (this.block == Blocks.AIR) {
            ...
            WorldUtilities.setBlockstate(
               world, this.p, this.blockState.withProperty(BlockFlowerPot.CONTENTS, EnumFlowerType.values()[this.meta]), notifyBlocks, playSound
            );
         } else {
      ...
      if (this.block instanceof BlockDoor) {
         if (this.blockState.getValue(BlockDoor.HALF) == EnumDoorHalf.LOWER) {
            IBlockState bs = this.blockState.withProperty(BlockDoor.HALF, EnumDoorHalf.UPPER);
         ...
         IBlockState bs = this.blockState.withProperty(BlockDoublePlant.HALF, EnumBlockHalf.UPPER);
         WorldUtilities.setBlockstate(world, this.p.getAbove(), bs, notifyBlocks, playSound);
      } else if (this.block instanceof BlockFlowerPot) {
         ...
         if (te instanceof TileEntityFlowerPot) {
            ...
            teFlowerPot.setItemStack(BlockItemUtilities.getFlowerpotItemStackFromEnum(EnumFlowerType.values()[this.meta]));
            teFlowerPot.markDirty();
            world.notifyBlockUpdate(
               this.p.getBlockPos(), this.blockState, this.blockState.withProperty(BlockFlowerPot.CONTENTS, EnumFlowerType.values()[this.meta]), 3
            );
         }
            ...
            try {
               if (townHall == null) {
                  ItemStack bannerStack = ItemMockBanner.makeBanner(
                     Item.getItemFromBlock(bannerBlock),
                  ...
                  bannerStack.getOrCreateSubCompound("BlockEntityTag")
                     .setTag("Base", new NBTTagInt(ItemMockBanner.BANNER_COLOURS[bannerBlock.bannerType].getDyeDamage()));
   ...
   private boolean buildPicture(World world) {
      ...
      if (art.onValidSurface() && !world.isRemote) {
         world.spawnEntity(art);
         return true;
      } else {
         return false;
      }
   ...
   private boolean buildPreserveGround(World world, boolean worldGeneration, boolean notifyBlocks, boolean playSound) {
      ...
      if (targetGroundBlockState == existingBlockState) {
         return false;
      } else if (existingBlockState.getBlock() == Blocks.GRASS && targetGroundBlockState.getBlock() == Blocks.DIRT) {
         return false;
      } else {
   ...
   private boolean buildTreeSpawn(World world, boolean worldGeneration) {
      if (worldGeneration) {
         ...
         if (this.special == OAKSPAWN) {
            ...
            IBlockState iblockstate = Blocks.LOG.getDefaultState().withProperty(BlockOldLog.VARIANT, EnumType.JUNGLE);
   ...
   private EnumFacing guessChestFurnaceFacing(World world, Point p) {
      ...
      if (bsNorth.isOpaqueCube()
         ...
         return EnumFacing.SOUTH;
      } else if (!bsNorth.isOpaqueCube()) {
         return EnumFacing.NORTH;
      } else if (!bsEast.isOpaqueCube()) {
         return EnumFacing.EAST;
      } else {
         return !bsWest.isOpaqueCube() ? EnumFacing.WEST : EnumFacing.NORTH;
      }
   }
