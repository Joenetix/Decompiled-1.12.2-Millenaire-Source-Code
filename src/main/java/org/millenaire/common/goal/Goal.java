...
public abstract class Goal {
   ...
   public final boolean isPossible(MillVillager villager) {
      try {
         if (villager.world.isDaytime() && !this.canBeDoneInDayTime()) {
            return false;
         } else if (!villager.world.isDaytime() && !this.canBeDoneAtNight()) {
            return false;
         } else if (this.minimumHour >= 0 && villager.world.getWorldTime() % 24000L < this.minimumHour) {
            return false;
         } else if (this.maximumHour >= 0 && villager.world.getWorldTime() % 24000L > this.maximumHour) {
            return false;
         } else {
