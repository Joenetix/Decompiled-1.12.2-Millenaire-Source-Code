...
public abstract class GoalGeneric extends Goal implements ParametersManager.DefaultValueOverloaded {
    ...
    // Subclasses will be added here as they are ported
            ...
            try {
                ...
                for (File file : getGenericGoalFiles("generic" + goalType)) {
                    try {
                        ...
                        goal.file = file;
                        ParametersManager.loadAnnotedParameterData(file, goal, null,
                                "generic " + goal.getTypeLabel() + " goal", null);
    ...
    public abstract Goal.GoalInformation getDestination(MillVillager villager) throws Exception;
    ...
    // Replacing validateDest which should be in Goal? No, validateDest is in Goal?
    // In decompiled: validateDest(villager, dest) is called.
    ...
    public final boolean isPossibleSpecific(MillVillager villager) throws Exception {
        if (this.reoccurDelay > 0
                ...
                && villager.lastGoalTime.get(this) + this.reoccurDelay > villager.getLevel().getGameTime()) {
            return false;
    ...
    public int priority(MillVillager villager) throws Exception {
        return this.priority + villager.getRandom().nextInt(this.priorityRandom + 1);
    }
    ...
    public void applyDefaultSettings() {
        // Default implementation if needed, or leave abstract if interface requires it?
        // Interface DefaultValueOverloaded has void applyDefaultSettings();
    }
}
