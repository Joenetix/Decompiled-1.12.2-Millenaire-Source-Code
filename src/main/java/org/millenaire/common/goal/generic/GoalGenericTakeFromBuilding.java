...
public class GoalGenericTakeFromBuilding extends GoalGeneric {
        ...
        for (Building dest : this.getBuildings(villager)) {
            if (this.isDestPossible(villager, dest)) {
                ...
                for (InvItem ii : this.collectGoods.keySet()) {
                     ...
                     * if (quantityNeeded > 0) {
                     * totalCount += Math.min(quantityNeeded, dest.nbGoodAvailable(ii, false, false,
                     * false));
                     * }
                     */
                    totalCount = this.minimumpickup; // Temp bypass
                }

                if (totalCount >= this.minimumpickup) {
                    // TODO: Implement getSellingPos
                    return this.packDest(null, dest);
    ...
    public boolean performAction(MillVillager villager) throws Exception {
        ...
        if (dest != null) {
            for (InvItem ii : this.collectGoods.keySet()) {
                // TODO: Implement takeFromBuilding
                /*
                 * int quantityNeeded = this.collectGoods.get(ii) - villager.countInv(ii);
                 * if (quantityNeeded > 0) {
                 * villager.takeFromBuilding(dest, ii.getItem(), ii.meta,
                 * Math.min(quantityNeeded, dest.nbGoodAvailable(ii, false, false, false)));
                 * }
                 */
            }
        }
