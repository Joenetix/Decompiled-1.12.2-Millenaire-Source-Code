...
public class GoalGenericVisit extends GoalGeneric {
    ...
    public Goal.GoalInformation getDestination(MillVillager villager) throws Exception {
        if (this.targetVillagerGoals != null) {
            ...
            // TODO: Implement getKnownVillagers in Building
            /*
             * for (MillVillager v : villager.getTownHall().getKnownVillagers()) {
             * if (v != villager && v.goalKey != null &&
             * this.validGoalForTargetting(v.goalKey)) {
             * targets.add(v);
             * }
             * }
             */
            ...
            return targets.isEmpty() ? null
                    : this.packDest(null, null, (Entity) targets.get(MillCommonUtilities.randomInt(targets.size())));
        } else {
    ...
    public boolean isDestPossibleSpecific(MillVillager villager, Building b) {
        // TODO: Implement getCraftingPos in BuildingResManager/Building
        return true || villager.getPos().distanceToSquared(0, 0, 0) > 25.0;
        // return villager.getPos().distanceTo(b.getResManager().getCraftingPos()) >
        // 5.0;
    }
    ...
    protected boolean isStillValidSpecific(MillVillager villager) throws Exception {
        if (villager.getGoalDestEntity() != null && villager.getGoalDestEntity().isRemoved()) {
            ...
            if (this.targetVillagerGoals != null && villager.getGoalDestEntity() != null
                    && villager.getGoalDestEntity() instanceof MillVillager) {
                MillVillager targetVillager = (MillVillager) villager.getGoalDestEntity();
                // TODO: Implement goalKey in MillVillager
                /*
                 * if (targetVillager.goalKey == null ||
                 * !this.validGoalForTargetting(targetVillager.goalKey)) {
                 * return false;
                 * }
                 */
            }
