...
public class ServerSender {
   ...
   public static void displayVillageTradeGUI(EntityPlayer player, Building building) {
      ...
      sendPacketToPlayer(data, player);
      player.openGui(Mill.instance, 2, player.world, building.getPos().getiX(), building.getPos().getiY(),
            building.getPos().getiZ());
   }
   ...
   public static void sendContentUnlocked(EntityPlayer player, int contentType, String cultureKey, String contentKey,
   ...
   public static void sendContentUnlockedMultiple(
         EntityPlayer player, int contentType, String cultureKey, List<String> contentKeys, int nbUnlocked,
         int nbTotal) {
      if (player != null) {
         if (player instanceof EntityPlayerMP) {
            PacketBuffer data = getPacketBuffer();
   ...
   public static void sendPacketToPlayersInRange(PacketBuffer packetBuffer, Point p, int range) {
      FMLProxyPacket proxyPacket = new FMLProxyPacket(packetBuffer, "millenaire");
      TargetPoint tp = new TargetPoint(0, p.x, p.y, p.z, range);
   ...
   public static void sendTranslatedSentenceInRange(World world, Point p, int range, char colour, String key,
         String... values) {
