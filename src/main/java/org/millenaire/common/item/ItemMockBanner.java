...
public class ItemMockBanner extends ItemBlock {
   ...
   private final BlockBannerHanging wallBanner;
   ...
   public static ItemStack makeBanner(Item banner, EnumDyeColor color, @Nullable NBTTagCompound patterns) {
      ...
      if (patterns != null && !patterns.isEmpty()) {
         itemstack.getOrCreateSubCompound("BlockEntityTag").setTag("Patterns", patterns.copy().getTagList("Patterns", 10));
      }

      return itemstack;
   }

   public ItemMockBanner(BlockBanner standingBanner, BlockBannerHanging wallBanner, EnumDyeColor color, int design) {
      super(standingBanner);
   ...
   public void getSubItems(CreativeTabs tab, NonNullList<ItemStack> items) {
      if (this.isInCreativeTab(tab)) {
         try {
            items.add(makeBanner(this, this.color, JsonToNBT.getTagFromJson(BANNER_DESIGNS[this.bannerDesign])));
         } catch (NBTException var4) {
            items.add(makeBanner(this, this.color, null));
         }
      }
   }
      ...
      if (facing != EnumFacing.DOWN && (iblockstate.getMaterial().isSolid() || flag) && (!flag || facing == EnumFacing.UP)) {
         ...
         if (player.canPlayerEdit(pos, facing, itemstack) && this.block.canPlaceBlockAt(worldIn, pos)) {
            ...
            if (facing == EnumFacing.UP) {
               int i = MathHelper.floor((player.rotationYaw + 180.0F) * 16.0F / 360.0F + 0.5) & 15;
               worldIn.setBlockState(pos, this.block.getDefaultState().withProperty(BlockStandingSign.ROTATION, i), 3);
            } else {
               worldIn.setBlockState(pos, this.wallBanner.getDefaultState().withProperty(BlockWallSign.FACING, facing), 3);
            }
            ...
            if (bannerEntity instanceof TileEntityMockBanner) {
               ...
               bannerStack.getOrCreateSubCompound("BlockEntityTag").setTag("Base", new NBTTagInt(this.color.getDyeDamage()));
