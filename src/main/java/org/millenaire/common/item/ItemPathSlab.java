...
public class ItemPathSlab extends ItemBlock {
   ...
   public boolean canPlaceBlockOnSide(World worldIn, BlockPos pos, EnumFacing side, EntityPlayer player,
      ...
      return iblockstate1.getBlock() == this.singleSlab ? true
            : super.canPlaceBlockOnSide(worldIn, pos, side, player, stack);
   }

   protected <T extends Comparable<T>> IBlockState makeDoubleState(IProperty<T> p_185055_1_,
      ...
      if (!itemstack.isEmpty() && player.canPlayerEdit(pos.offset(facing), facing, itemstack)) {
         ...
         if (iblockstate.getBlock() == this.singleSlab) {
            ...
            if (facing == EnumFacing.UP && blockslab$enumblockhalf == EnumBlockHalf.BOTTOM
               ...
               IBlockState doubleBlockState = this.doubleSlab.getDefaultState().withProperty(IBlockPath.STABLE, true);
               AxisAlignedBB axisalignedbb = doubleBlockState.getCollisionBoundingBox(worldIn, pos);
               if (axisalignedbb != Block.NULL_AABB
                     && worldIn.checkNoEntityCollision(axisalignedbb.offset(pos))
                     && worldIn.setBlockState(pos, doubleBlockState, 11)) {
                  SoundType soundtype = this.doubleSlab.getSoundType(doubleBlockState, worldIn, pos, player);
                  worldIn.playSound(
                        player, pos, soundtype.getPlaceSound(), SoundCategory.BLOCKS,
   ...
   private boolean tryPlace(EntityPlayer player, ItemStack stack, World worldIn, BlockPos pos) {
      IBlockState iblockstate = worldIn.getBlockState(pos);
      if (iblockstate.getBlock() == this.singleSlab) {
         IBlockState iblockstate1 = this.doubleSlab.getDefaultState().withProperty(IBlockPath.STABLE, true);
         ...
         if (axisalignedbb != Block.NULL_AABB && worldIn.checkNoEntityCollision(axisalignedbb.offset(pos))
            ...
            worldIn.playSound(
                  player, pos, soundtype.getPlaceSound(), SoundCategory.BLOCKS, (soundtype.getVolume() + 1.0F) / 2.0F,
