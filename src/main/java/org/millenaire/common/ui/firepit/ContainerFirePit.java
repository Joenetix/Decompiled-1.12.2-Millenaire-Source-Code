...
public class ContainerFirePit extends Container {
   ...
   public ContainerFirePit(EntityPlayer player, TileEntityFirePit firePit) {
      ...
      for (int i = 0; i < 3; i++) {
         this.addSlotToContainer(new SlotFirePitInput(firePit.inputs, i, INPUT_POSITIONS[i][0], INPUT_POSITIONS[i][1]));
      }

      this.inputEnd = this.inventorySlots.size();
      this.fuelStart = this.inventorySlots.size();
      this.addSlotToContainer(new SlotFirePitFuel(firePit.fuel, 0, FUEL_POSITION[0], FUEL_POSITION[1]));
      this.fuelEnd = this.inventorySlots.size();
      this.outputStart = this.inventorySlots.size();

      for (int i = 0; i < 3; i++) {
         this.addSlotToContainer(new SlotFirePitOutput(player, firePit.outputs, i, OUTPUT_POSITIONS[i][0], OUTPUT_POSITIONS[i][1]));
      }
      ...
      for (int row = 0; row < 3; row++) {
         for (int column = 0; column < 9; column++) {
            this.addSlotToContainer(new Slot(playerInventory, column + row * 9 + 9, INV_POSITION[0] + column * 18, INV_POSITION[1] + row * 18));
         }
      }
      ...
      for (int hotbarIndex = 0; hotbarIndex < 9; hotbarIndex++) {
         this.addSlotToContainer(new Slot(playerInventory, hotbarIndex, INV_POSITION[0] + hotbarIndex * 18, INV_POSITION[1] + 54 + 4));
      }
   ...
   public ItemStack transferStackInSlot(EntityPlayer playerIn, int index) {
      ...
      if (slot != null && slot.getHasStack()) {
         ...
         if (inRange(index, this.outputStart, this.outputEnd)) {
            if (!this.mergeItemStack(stackInSlot, this.inventoryStart, this.hotbarEnd, true)) {
            ...
            if (TileEntityFirePit.isFirePitBurnable(stackInSlot)) {
               if (!this.mergeItemStack(stackInSlot, this.inputStart, this.inputEnd, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (TileEntityFurnace.isItemFuel(stackInSlot)) {
               if (!this.mergeItemStack(stackInSlot, this.fuelStart, this.fuelEnd, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (inRange(index, this.inventoryStart, this.inventoryEnd)) {
               if (!this.mergeItemStack(stackInSlot, this.hotbarStart, this.hotbarEnd, false)) {
                  return ItemStack.EMPTY;
               }
            } else if (!this.mergeItemStack(stackInSlot, this.inventoryStart, this.inventoryStart, false)) {
               return ItemStack.EMPTY;
            }
         } else if (!this.mergeItemStack(stackInSlot, this.inventoryStart, this.hotbarEnd, false)) {
            return ItemStack.EMPTY;
         }
