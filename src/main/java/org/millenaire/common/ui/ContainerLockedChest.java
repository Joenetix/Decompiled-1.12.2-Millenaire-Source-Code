...
public class ContainerLockedChest extends Container {
   ...
   public ContainerLockedChest(IInventory playerInventory, IInventory chestInventory, EntityPlayer player, Building building, boolean locked) {
      ...
      for (int j = 0; j < this.numRows; j++) {
         for (int k = 0; k < 9; k++) {
            if (locked) {
               this.addSlotToContainer(new ContainerLockedChest.LockedSlot(chestInventory, k + j * 9, 8 + k * 18, 18 + j * 18));
            } else {
               this.addSlotToContainer(new ContainerLockedChest.CachedSlot(chestInventory, k + j * 9, 8 + k * 18, 18 + j * 18, building));
            }
         }
      }
   ...
   public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
      if (slotId >= 0 && slotId < this.inventorySlots.size()) {
         ...
         if (slot != null && slot instanceof ContainerLockedChest.LockedSlot && this.locked) {
