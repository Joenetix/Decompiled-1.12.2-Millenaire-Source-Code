...
public class TileEntityLockedChest extends TileEntityLockableLoot implements ITickable, ISidedInventory {
   ...
   public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
      ...
      return new ContainerLockedChest(playerInventory, this, playerIn, building, this.isLockedFor(playerIn));
   }

   @Nullable
   protected TileEntityLockedChest getAdjacentLockedChest(EnumFacing side) {
      BlockPos blockpos = this.pos.offset(side);
      if (this.isLockedChestAt(blockpos)) {
      ...
      if (this.buildingPos == null) {
         ...
         if (building == null) {
            ...
            return building.chestLocked
               ? new TextComponentString(s + ": " + LanguageUtilities.string("ui.lockedchest"))
               : new TextComponentString(s + ": " + LanguageUtilities.string("ui.unlockedchest"));
         }
      }
   }
   ...
   public boolean hasCapability(Capability<?> capability, @Nullable EnumFacing facing) {
      return capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY ? false : super.hasCapability(capability, facing);
   }
   ...
   public boolean isLockedFor(EntityPlayer player) {
      if (player == null) {
         ...
         return true;
      } else if (!this.loaded && this.world.isRemote) {
         return true;
      } else if (this.buildingPos == null) {
         return false;
      } else if (!this.world.isRemote && MillConfigValues.DEV) {
         return false;
         ...
         if (mw == null) {
            ...
            return building == null ? true : building.lockedForPlayer(player);
         }
      }
   }
      ...
      if (!this.world.isRemote && this.numPlayersUsing != 0 && (this.ticksSinceSync + i + j + k) % 200 == 0) {
         ...
         for (EntityPlayer entityplayer : this.world
            ...
            if (entityplayer.openContainer instanceof ContainerLockedChest) {
               ...
               if (iinventory == this
                  || iinventory instanceof TileEntityLockedChest.InventoryLockedLargeChest
                     && ((TileEntityLockedChest.InventoryLockedLargeChest)iinventory).isPartOfLargeChest(this)) {
                  this.numPlayersUsing++;
               }
            }
         }
   ...
   public static class InventoryLockedLargeChest implements ILockableContainer, ISidedInventory {
      ...
      public Container createContainer(InventoryPlayer playerInventory, EntityPlayer playerIn) {
         ...
         return new ContainerLockedChest(playerInventory, this, playerIn, building, this.upperChest.isLockedFor(playerIn));
      }
