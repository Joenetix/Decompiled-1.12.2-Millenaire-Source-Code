...
public class VillagerRecord implements Cloneable {
   ...
   public static VillagerRecord createVillagerRecord(
      ...
      if (mockVillager || !worldData.world.isRemote && worldData.world instanceof WorldServer) {
   ...
   public static VillagerRecord read(MillWorldData mw, NBTTagCompound nbttagcompound, String label) {
      if (!nbttagcompound.hasKey(label + "_id") && !nbttagcompound.hasKey(label + "_lid")) {
         ...
         for (int i = 0; i < nbttaglist.tagCount(); i++) {
            ...
            vr.inventory
               .put(
                  InvItem.createInvItem(Item.getItemById(nbttagcompound1.getInteger("item")), nbttagcompound1.getInteger("meta")),
   ...
   public VillagerRecord generateRaidRecord(Building target) {
      ...
      raidRecord.raiderSpawn = this.getTownHall().world.getWorldTime();
      return raidRecord;
   }
   ...
   public void write(NBTTagCompound nbttagcompound, String label) {
      nbttagcompound.setLong(label + "_lid", this.getVillagerId());
      ...
      nbttagcompound.setString(label + "_culture", this.getCulture().key);
   }
}
