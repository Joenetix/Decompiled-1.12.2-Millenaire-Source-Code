...
public class SpecialQuestActions {
   ...
   private static void indianCQHandleBottomOfTheWorld(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_bottomoftheworld")
            && !mw.getProfile(player).isTagSet("action_bottomoftheworld_complete")) {
         if (player.posY < 4.0) {
   ...
   private static void indianCQHandleContinuousExplore(
         MillWorldData mw, EntityPlayer player, long worldTime, String biome, ResourceLocation mob, int nbMob,
         int minTravel) throws IllegalArgumentException, IllegalAccessException {
      if (mw.getProfile(player).isTagSet("action_explore_" + biome)
            && !mw.getProfile(player).isTagSet("action_explore_" + biome + "_complete")) {
         if (!mw.world.isDaytime()) {
            ...
            if (biomeName.equals(biome)) {
               ...
               if (!(player.posY <= surface - 2)) {
                  ...
                  if (++nbtest < 20) {
                     mw.getProfile(player).setActionData(biome + "_explore_point" + nbtest,
                           new Point(player).getIntString());
                     mw.getProfile(player).setActionData(biome + "_explore_nbcomplete", "" + nbtest);
                     ServerSender.sendTranslatedSentence(player, '7', "actions." + biome + "_continue",
                           "" + nbtest * 5);
   ...
   private static void indianCQHandleEnchantmentTable(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_build_enchantment_table")
            && !mw.getProfile(player).isTagSet("action_build_enchantment_table_complete")) {
         boolean closeEnough = false;

         for (int i = 0; i < mw.loneBuildingsList.types.size(); i++) {
            if (mw.loneBuildingsList.types.get(i).equals("sadhutree")
                  && mw.loneBuildingsList.pos.get(i).distanceToSquared(player) < 100.0) {
               closeEnough = true;
            }
         }
                     ...
                     if (block == Blocks.ENCHANTING_TABLE) {
                        ...
                        for (int dx = -1; dx <= 1; dx++) {
                           for (int dz = -1; dz <= 1; dz++) {
                              if ((dx != 0 || dz != 0)
                                    && mw.world.isAirBlock(new BlockPos(x + dx, y, z + dz))
                                    && mw.world.isAirBlock(new BlockPos(x + dx, y + 1, z + dz))) {
                                 if (WorldUtilities.getBlock(mw.world, x + dx * 2, y, z + dz * 2) == Blocks.BOOKSHELF) {
                                    nbBookShelves++;
                                 }

                                 if (WorldUtilities.getBlock(mw.world, x + dx * 2, y + 1,
                                       z + dz * 2) == Blocks.BOOKSHELF) {
                                    nbBookShelves++;
                                 }
                                 ...
                                 if (dz != 0 && dx != 0) {
                                    ...
                                    if (WorldUtilities.getBlock(mw.world, x + dx, y + 1,
                                          z + dz * 2) == Blocks.BOOKSHELF) {
                                       nbBookShelves++;
                                    }
                                 }
   ...
   private static void indianCQHandleTopOfTheWorld(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_topoftheworld")
            && !mw.getProfile(player).isTagSet("action_topoftheworld_complete")) {
         if (player.posY > 250.0) {
   ...
   private static void indianCQHandleUnderwaterDive(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_underwater_dive")
            && !mw.getProfile(player).isTagSet("action_underwater_dive_complete")) {
   ...
   private static void indianCQHandleUnderwaterGlass(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_underwater_glass")
            && !mw.getProfile(player).isTagSet("action_underwater_glass_complete")) {
         Point p = new Point(player);

         for (Block block = WorldUtilities.getBlock(mw.world, p); block != null
               && !BlockItemUtilities.isBlockOpaqueCube(block) && block != Blocks.GLASS
               && block != Blocks.GLASS_PANE; block = WorldUtilities.getBlock(mw.world, p)) {
            p = p.getAbove();
         }
   ...
   private static void mayanCQHandleMayanSiege(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_mayansiege")
            && !mw.getProfile(player).isTagSet("action_mayansiege_complete")) {
         ...
         if (siegeStatus == null) {
            for (Point p : mw.loneBuildingsList.pos) {
               ...
               if (b != null && b.villageType.key.equals("questpyramid") && p.distanceTo(player) < 50.0) {
                  ...
                  for (int i = 0; i < 12; i++) {
                     Point spawn = b.location.pos.getRelative(-10 + MillCommonUtilities.randomInt(20), 20.0,
                           -10 + MillCommonUtilities.randomInt(20));
                     EntityTargetedGhast ent = (EntityTargetedGhast) WorldUtilities.spawnMobsSpawner(mw.world, spawn,
                  ...
                  for (int ix = 0; ix < 12; ix++) {
                     Point spawn = b.location.pos.getRelative(-5 + MillCommonUtilities.randomInt(10), 15.0,
                           -5 + MillCommonUtilities.randomInt(10));
                     EntityTargetedBlaze ent = (EntityTargetedBlaze) WorldUtilities.spawnMobsSpawner(mw.world, spawn,
                  ...
                  ServerSender.sendTranslatedSentence(player, '7', "actions.mayan_siege_start", "" + nbGhasts,
                        "" + nbBlazes, "" + nbSkel);
               }
            }
            ...
            for (Point px : mw.loneBuildingsList.pos) {
               ...
               if (b != null && b.villageType.key.equals("questpyramid") && px.distanceTo(player) < 50.0) {
                  List<Entity> mobs = WorldUtilities.getEntitiesWithinAABB(mw.world, EntityTargetedGhast.class,
                        b.location.pos, 128, 128);
                  int nbGhasts = mobs.size();
                  mobs = WorldUtilities.getEntitiesWithinAABB(mw.world, EntityTargetedBlaze.class, b.location.pos, 128,
                        128);
                  int nbBlazes = mobs.size();
                  mobs = WorldUtilities.getEntitiesWithinAABB(mw.world, EntityTargetedWitherSkeleton.class,
                        b.location.pos, 128, 128);
                  ...
                  if (nbGhasts == 0 && nbBlazes == 0 && nbSkel == 0) {
                     ...
                     if (oldGhasts != nbGhasts || oldBlazes != nbBlazes || oldSkel != nbSkel) {
                        ServerSender.sendTranslatedSentence(player, '7', "actions.mayan_siege_update", "" + nbGhasts,
                              "" + nbBlazes, "" + nbSkel);
                        mw.getProfile(player).setActionData("mayan_siege_ghasts", "" + nbGhasts);
                        mw.getProfile(player).setActionData("mayan_siege_blazes", "" + nbBlazes);
                        mw.getProfile(player).setActionData("mayan_siege_skeletons", "" + nbSkel);
   ...
   private static void normanCQHandleBorehole(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_borehole")
            && !mw.getProfile(player).isTagSet("action_borehole_complete")) {
         if (!(player.posY > 10.0)) {
            int nbok = 0;

            for (int x = (int) (player.posX - 2.0); x < (int) player.posX + 3; x++) {
               for (int z = (int) (player.posZ - 2.0); z < (int) player.posZ + 3; z++) {
                  boolean ok = true;
                  boolean stop = false;
   ...
   private static void normanCQHandleBoreholeTNT(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_boreholetnt")
            && !mw.getProfile(player).isTagSet("action_boreholetnt_complete")) {
   ...
   private static void normanCQHandleBoreholeTNTLit(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_boreholetntlit")
            && !mw.getProfile(player).isTagSet("action_boreholetntlit_complete")) {
         Point p = new Point(mw.getProfile(player).getActionData("action_borehole_pos"));
         int nbtnt = mw.world
               .getEntitiesWithinAABB(EntityTNTPrimed.class,
                     new AxisAlignedBB(p.x, p.y, p.z, p.x + 1.0, p.y + 1.0, p.z + 1.0).expand(8.0, 4.0, 8.0))
               .size();
         if (nbtnt > 0) {
   ...
   private static void normanCQHandleTheVoid(MillWorldData mw, EntityPlayer player) {
      if (mw.getProfile(player).isTagSet("action_thevoid")
            && !mw.getProfile(player).isTagSet("action_thevoid_complete")) {
         if (!(player.posY > 30.0)) {
            for (int i = -5; i < 5; i++) {
               for (int j = -5; j < 5; j++) {
   ...
   public static void normanMarvelPickLocation(MillWorldData mw, EntityPlayer player, Point pos) {
      ...
      if (closestVillageDistance == Double.MAX_VALUE) {
         ...
         ServerSender.sendTranslatedSentence(player, '6', "actions.normanmarvel_villagetooclose", "200",
               "" + Math.round(closestVillageDistance));
      }
   }
   ...
   public static void onTick(MillWorldData mw, EntityPlayer player) {
      ...
      for (long worldTime = startTime; worldTime <= mw.world.getWorldTime(); worldTime++) {
         if (worldTime % 250L == 0L) {
            try {
               indianCQHandleContinuousExplore(mw, player, worldTime, MillConfigValues.questBiomeForest,
                     Mill.ENTITY_ZOMBIE, 2, 15);
               indianCQHandleContinuousExplore(mw, player, worldTime, MillConfigValues.questBiomeDesert,
                     Mill.ENTITY_SKELETON, 2, 15);
               indianCQHandleContinuousExplore(mw, player, worldTime, MillConfigValues.questBiomeMountain,
                     Mill.ENTITY_SPIDER, 2, 10);
