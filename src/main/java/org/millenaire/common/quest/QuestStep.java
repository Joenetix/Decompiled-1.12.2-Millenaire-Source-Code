   ...
   public String lackingConditions(EntityPlayer player) {
      ...
      for (InvItem item : this.requiredGood.keySet()) {
         ...
         if (item.special == 1) {
            ...
            diff = this.requiredGood.get(item) - MillCommonUtilities.countChestItems(player.inventory, item.getItem(), item.meta);
         } else {
            int nbenchanted = 0;

            for (int ix = 0; ix < player.inventory.getSizeInventory(); ix++) {
               ItemStack stack = player.inventory.getStackInSlot(ix);
