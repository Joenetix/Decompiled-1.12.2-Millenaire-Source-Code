...
public class LayerVillagerClothes implements LayerRenderer<EntityLivingBase> {
   ...
   private void renderClothLayer(
      MillVillager villager, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch, float scale
   ) {
      if (villager.getClothTexturePath(this.layer) != null) {
         this.modelCloth.setModelAttributes(this.renderer.getMainModel());
         this.modelCloth.setLivingAnimations(villager, limbSwing, limbSwingAmount, partialTicks);
         this.renderer.bindTexture(villager.getClothTexturePath(this.layer));
         GlStateManager.color(1.0F, 1.0F, 1.0F, 1.0F);
         this.modelCloth.render(villager, limbSwing, limbSwingAmount, ageInTicks, netHeadYaw, headPitch, scale);
      }
   }
