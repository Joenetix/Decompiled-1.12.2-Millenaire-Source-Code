...
public class RenderWallDecoration extends Render<EntityWallDecoration> {
   public static final RenderWallDecoration.FactoryRenderWallDecoration FACTORY_WALL_DECORATION = new RenderWallDecoration.FactoryRenderWallDecoration();
   ...
   protected ResourceLocation getEntityTexture(EntityWallDecoration entity) {
      return entity.type == 1 ? textureTapestries : textureSculptures;
   }
   ...
   private void renderPainting(EntityWallDecoration painting, int width, int height, int textureU, int textureV) {
      ...
      for (int i = 0; i < width / 16; i++) {
         for (int j = 0; j < height / 16; j++) {
            ...
            bufferbuilder.pos(f16, f18, -0.5).tex(0.8125, 0.001953125).normal(0.0F, -1.0F, 0.0F).endVertex();
            bufferbuilder.pos(f15, f18, -0.5).tex(0.75, 0.001953125).normal(0.0F, -1.0F, 0.0F).endVertex();
            bufferbuilder.pos(f15, f17, 0.5).tex(0.7519531F, 0.0).normal(-1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f15, f18, 0.5).tex(0.7519531F, 0.0625).normal(-1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f15, f18, -0.5).tex(0.7519531F, 0.0625).normal(-1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f15, f17, -0.5).tex(0.7519531F, 0.0).normal(-1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f16, f17, -0.5).tex(0.7519531F, 0.0).normal(1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f16, f18, -0.5).tex(0.7519531F, 0.0625).normal(1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f16, f18, 0.5).tex(0.7519531F, 0.0625).normal(1.0F, 0.0F, 0.0F).endVertex();
            bufferbuilder.pos(f16, f17, 0.5).tex(0.7519531F, 0.0).normal(1.0F, 0.0F, 0.0F).endVertex();
            tessellator.draw();
         }
