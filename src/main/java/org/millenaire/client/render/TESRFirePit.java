...
public class TESRFirePit extends TileEntitySpecialRenderer<TileEntityFirePit> {
   private static final TRSRTransformation[] COOKING_POSITIONS = new TRSRTransformation[]{
      get(0.5F, 1.0F, 0.4F, 25.0F, 180.0F, -45.0F, 0.35F),
      get(0.5F, 0.9F, 0.5F, 0.0F, 45.0F, -45.0F, 0.35F),
      get(0.5F, 1.0F, 0.6F, -25.0F, 180.0F, -45.0F, 0.35F)
   };
   private static final TRSRTransformation[] COOKED_POSITIONS = new TRSRTransformation[]{
      get(0.5F, 0.9F, 0.4F, 25.0F, 180.0F, -45.0F, 0.35F),
      get(0.5F, 0.9F, 0.5F, 0.0F, -45.0F, -45.0F, 0.35F),
   ...
   private static void render(ItemStack stack, RenderItem item) {
      item.renderItem(stack, TransformType.FIXED);
   }
   ...
   public void render(TileEntityFirePit te, double x, double y, double z, float partialTicks, int destroyStage, float alpha) {
      ...
      if (firePitBS.getBlock() instanceof BlockFirePit) {
         double alignment = ((BlockFirePit.EnumAlignment)firePitBS.getValue(BlockFirePit.ALIGNMENT)).angle;
         GlStateManager.pushMatrix();
         ...
         if (!fuel.isEmpty()) {
            ...
            int posTotal = te.getPos().getX() + te.getPos().getY() + te.getPos().getZ() & 3;
