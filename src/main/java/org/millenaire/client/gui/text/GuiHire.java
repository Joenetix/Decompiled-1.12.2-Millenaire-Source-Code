...
public class GuiHire extends GuiText {
   ...
   public void buttonPagination() {
      ...
      if (this.villager.hiredBy != null) {
         if (MillCommonUtilities.countMoney(this.player.inventory) >= this.villager.getHireCost(this.player)) {
            this.buttonList
               .add(
         ...
         if (this.villager.getTownHall().getReputation(this.player) >= 4096
            && MillCommonUtilities.countMoney(this.player.inventory) >= this.villager.getHireCost(this.player)) {
            this.buttonList
               .add(
   ...
   private TextBook getData() {
      ...
      text.add(
         new TextLine(this.villager.getName() + ", " + this.villager.getNativeOccupationName(), "§1", new GuiText.GuiButtonReference(this.villager.vtype))
      );
         ...
         text.add(
            new TextLine(
               LanguageUtilities.string(
                  "hire.hiredvillager",
                  "" + Math.round((float)((this.villager.hiredUntil - this.villager.world.getWorldTime()) / 1000L)),
