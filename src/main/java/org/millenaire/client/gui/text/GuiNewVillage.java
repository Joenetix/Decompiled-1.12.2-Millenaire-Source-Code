...
public class GuiNewVillage extends GuiText {
   ...
   public GuiNewVillage(EntityPlayer player, Point p) {
      System.out.println("Millenaire DEBUG: Opening GuiNewVillage at " + p);
      this.pos = p;
      this.player = player;
   ...
   public void initData() {
      ...
      for (Culture culture : Culture.ListCultures) {
         if (profile.isTagSet("culturecontrol_" + culture.key)) {
            text.add(new TextLine(LanguageUtilities.string("ui.leaderin", culture.getAdjectiveTranslated()),
                  new GuiText.GuiButtonReference(culture)));
         } else {
            text.add(new TextLine(LanguageUtilities.string("ui.notleaderin", culture.getAdjectiveTranslated()),
                  new GuiText.GuiButtonReference(culture)));
            notleader = true;
         }
      }
         ...
         text.add(
               new TextLine(
                     new GuiText.MillGuiButton(
                           this.possibleVillages.get(i).name, i, this.possibleVillages.get(i).culture.getIcon(),
                           this.possibleVillages.get(i).getIcon())));
