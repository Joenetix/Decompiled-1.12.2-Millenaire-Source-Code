...
public class ClientReceiver {
   ...
   public void onPacketData(ClientCustomPacketEvent event) {
      ...
      } else if (event.getPacket().payload() == null) {
   ...
   private void readAnimalBreedPacket(PacketBuffer data) {
      ...
      for (Entity ent : WorldUtilities.getEntitiesWithinAABB(Mill.clientWorld.world, EntityAnimal.class, p, 5, 5)) {
   ...
   private void readContentUnlockedPackage(PacketBuffer data) {
      ...
      if (culture != null) {
         if (contentType == 1) {
            ...
            if (planSet != null) {
               Minecraft.getMinecraft().getToastGui().add(new UnlockingToast(planSet, nbUnlocked, nbTotal));
            }
         } else if (contentType == 4) {
            TradeGood tradeGood = culture.getTradeGood(contentKey);
            if (tradeGood != null) {
               Minecraft.getMinecraft().getToastGui().add(new UnlockingToast(tradeGood, nbUnlocked, nbTotal));
            }
         } else if (contentType == 2) {
            VillageType villageType = culture.getVillageType(contentKey);
            if (villageType != null) {
               Minecraft.getMinecraft().getToastGui().add(new UnlockingToast(villageType, nbUnlocked, nbTotal));
            }
         } else if (contentType == 3) {
            VillagerType villagerType = culture.getVillagerType(contentKey);
            if (villagerType != null) {
               Minecraft.getMinecraft().getToastGui().add(new UnlockingToast(villagerType, nbUnlocked, nbTotal));
            }
         }
      }
   }
   ...
   private void readContentUnlockedPackageMultiple(PacketBuffer data) {
      ...
      if (culture != null && contentType == 5) {
         ...
         if (tradeGoods.size() > 0) {
            Minecraft.getMinecraft().getToastGui().add(new UnlockingToast(tradeGoods, nbUnlocked, nbTotal));
         }
      }
   }
