   ...
   public void getNextCandidates(AStarNode parent, boolean droppingAllowed) throws ThreadSafeUtilities.ChunkAccessException {
      ...
      for (int i = 0; i < c.length; i++) {
         ...
         try {
            ...
            if (!found && !this.tryToUpdateExistingHeapNode(parent, check) && AStarStatic.isViable(this.world, check, c[i][1], this.config)) {
               this.addToBinaryHeap(check);
            }
         } catch (Exception var12) {
            if (MillConfigValues.LogChunkLoader >= 2) {
               MillLog.minor(this, var12.getLocalizedMessage());
            }
         }
      }
   }
