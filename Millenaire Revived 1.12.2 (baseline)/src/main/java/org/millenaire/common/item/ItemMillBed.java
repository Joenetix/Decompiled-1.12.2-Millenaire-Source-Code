...
public class ItemMillBed extends ItemBlock {
   ...
   public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
      if (worldIn.isRemote) {
         ...
         boolean flag = placedBlock.isReplaceable(worldIn, pos);
         if (!flag) {
            pos = pos.up();
         }
         ...
         if (player.canPlayerEdit(pos, facing, itemstack) && player.canPlayerEdit(blockpos, facing, itemstack)) {
            ...
            if (flag2 && flag3 && worldIn.getBlockState(pos.down()).isTopSolid() && worldIn.getBlockState(blockpos.down()).isTopSolid()
             ...
             {
               IBlockState iblockstate2 = this.block
                  .getDefaultState()
                  .withProperty(BlockBed.OCCUPIED, false)
                  .withProperty(BlockHorizontal.FACING, enumfacing)
                  .withProperty(BlockBed.PART, EnumPartType.FOOT);
               worldIn.setBlockState(pos, iblockstate2, 10);
               worldIn.setBlockState(blockpos, iblockstate2.withProperty(BlockBed.PART, EnumPartType.HEAD), 10);
