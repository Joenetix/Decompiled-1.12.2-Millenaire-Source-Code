...
public class TileEntityImportTable extends TileEntity {
   ...
   public void activate(EntityPlayer player) {
      if (player.inventory.getCurrentItem() != ItemStack.EMPTY && player.inventory.getCurrentItem().getItem() == MillItems.SUMMONING_WAND) {
         ...
         && player.inventory.getCurrentItem().getItem() == MillItems.NEGATION_WAND) {
         if (player.world.isRemote && this.buildingKey != null) {
            BuildingImportExport.importTableExportBuildingPlan(player.world, this, this.getUpgradeLevel());
         } else {
            this.sendUpdates();
         }
      } else if (!player.world.isRemote) {
         this.sendUpdates();
         ServerSender.displayImportTableGUI(player, new Point(this.getPos()));
      }
   }
   ...
   private void updateAttachedSign() {
      Point signPos = new Point(this.pos).getRelative(-1.0, 0.0, 0.0);
      signPos.setBlockState(this.world, Blocks.WALL_SIGN.getDefaultState().withProperty(BlockWallSign.FACING, EnumFacing.WEST));
      TileEntitySign sign = signPos.getSign(this.world);
      if (this.buildingKey != null) {
         sign.signText[0] = new TextComponentString(this.buildingKey + "_" + (char)(65 + this.variation) + this.upgradeLevel);
      } else {
         sign.signText[0] = new TextComponentString("");
      }

      sign.signText[1] = new TextComponentString("Start level: " + this.startingLevel);
      sign.signText[2] = new TextComponentString(this.length + "x" + this.width);
      sign.markDirty();
