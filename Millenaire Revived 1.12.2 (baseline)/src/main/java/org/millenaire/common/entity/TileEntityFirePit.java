...
public class TileEntityFirePit extends TileEntity implements ITickable {
   private final ItemStackHandler items = new ItemStackHandler(7) {
      protected int getStackLimit(int slot, @Nonnull ItemStack stack) {
         return (0 > slot || slot >= 3 || TileEntityFirePit.isFirePitBurnable(stack))
               && (3 > slot || slot >= 4 || TileEntityFurnace.isItemFuel(stack))
               && (4 > slot || slot >= 7)
   ...
   public static boolean isFirePitBurnable(ItemStack stack) {
      ...
      return !result.isEmpty() && (food || result.getItem() instanceof ItemFood);
   }
      ...
      if (stack.isEmpty()) {
         ...
         if (result.isEmpty()) {
            ...
            return output.isEmpty()
               || ItemHandlerHelper.canItemStacksStack(result, output) && output.getCount() + result.getCount() <= result.getMaxStackSize();
         }
      }
   }
      ...
      for (int i = 0; i < this.items.getSlots(); i++) {
         ...
         if (!stack.isEmpty()) {
            InventoryHelper.spawnItemStack(
               this.world, this.pos.getX(), this.pos.getY(), this.pos.getZ(), stack
            );
         }
      }
   }
   ...
   public <T> T getCapability(@Nonnull Capability<T> capability, @Nullable EnumFacing facing) {
      if (capability == CapabilityItemHandler.ITEM_HANDLER_CAPABILITY) {
         if (facing == null) {
            ...
            return (T)(facing == EnumFacing.DOWN
               ? CapabilityItemHandler.ITEM_HANDLER_CAPABILITY.cast(this.outputs)
               : CapabilityItemHandler.ITEM_HANDLER_CAPABILITY.cast(this.fuel));
         }
      } else {
   ...
   public void update() {
      ...
      if (!this.world.isRemote) {
         ...
         if (burning != this.burnTime > 0) {
            ...
            this.world.setBlockState(this.pos, state.withProperty(BlockFirePit.LIT, this.burnTime > 0));
         }
      }
