   ...
   public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException {
      ...
      if (!world.isRemote) {
         ...
         if (args[0].equals("all")) {
            ...
            for (UserProfile profile : worldData.profiles.values()) {
               if (profile.connected) {
                  ...
                  if (sender instanceof EntityPlayer) {
                     ServerSender.sendTranslatedSentence((EntityPlayer)sender, '2', "Resent profile data for " + profile.playerName);
                  } else {
            ...
            if (sender instanceof EntityPlayer) {
               ServerSender.sendTranslatedSentence((EntityPlayer)sender, '2', "Resent profile data for " + profilex.playerName);
            } else {
