...
public class VillageMapInfo implements Cloneable {
   ...
   private void createWorldInfo(List<BuildingLocation> locations, int pstartX, int pstartZ, int endX, int endZ) throws MillLog.MillenaireException {
      ...
      this.frequency = (int)Math.max(1000.0 / (this.length * this.width / 256), 10.0);
      if (this.frequency == 0) {
         ...
         for (int i = 0; i < this.length; i += 16) {
            for (int j = 0; j < this.width; j += 16) {
               ...
               } catch (ThreadSafeUtilities.ChunkAccessException var9) {
   ...
   public boolean isConstructionForbiddenHere(Point p) {
      return p.getiX() >= this.mapStartX && p.getiZ() >= this.mapStartZ && p.getiX() < this.mapStartX + this.length && p.getiZ() < this.mapStartZ + this.width
         ? this.buildingForbidden[p.getiX() - this.mapStartX][p.getiZ() - this.mapStartZ]
         : false;
   }
   ...
   public boolean update(World world, List<BuildingLocation> locations, Point centre, int radius) throws MillLog.MillenaireException {
      ...
      if (this.buildingLocations != null && this.buildingLocations.size() > 0 && this.buildingLocations.size() == locations.size()) {
         ...
         if (MillConfigValues.LogWorldInfo >= 1) {
            MillLog.major(
               this, "Values after chunks: " + mapStartXTemp + "/" + mapStartZTemp + "/" + (mapStartXTemp + lengthTemp) + "/" + (mapStartZTemp + widthTemp)
            );
         }
         ...
         if (lengthTemp == this.length && widthTemp == this.width) {
            ...
            for (BuildingLocation locationx : locations) {
               this.addBuildingLocationToMap(locationx);
            }
   ...
   private void updateChunk(int startX, int startZ) throws ThreadSafeUtilities.ChunkAccessException {
      ...
      for (int i = 0; i < 16; i++) {
         for (int j = 0; j < 16; j++) {
            ...
            for (Block tblock = chunk.getBlockState(i, maxy, j).getBlock();
               y >= miny && !BlockItemUtilities.isBlockGround(tblock);
            ...
            while (
               y <= maxy
                  && y > 1
                  && (
                     BlockItemUtilities.isBlockSolid(chunk.getBlockState(i, y, j).getBlock())
                        || BlockItemUtilities.isBlockSolid(chunk.getBlockState(i, y + 1, j).getBlock())
                  )
            ) {
               y++;
            }
            ...
            if (soilBlock instanceof BlockLog) {
            ...
            if (!BlockItemUtilities.isFence(soilBlock)
               && !BlockItemUtilities.isBlockSolid(block)
            ...
            if (!this.danger[mx][mz]
               && this.buildingLocRef[mx][mz] == null
               && this.topGround[mx][mz] > this.yBaseline - 10
               && this.topGround[mx][mz] < this.yBaseline + 10) {
   ...
   public void updateNextChunk() {
      ...
      if (this.updateCounter == 0) {
         ...
         VillageMapInfo.UpdateThread thread = new VillageMapInfo.UpdateThread();
   ...
   public class UpdateThread extends Thread {
      ...
      public void run() {
         ...
         } catch (ThreadSafeUtilities.ChunkAccessException var2) {
            if (MillConfigValues.LogChunkLoader >= 2) {
               MillLog.minor(
                  this,
                  "Chunk access violation while attempting to update village map in thread at: "
                     + (this.x + VillageMapInfo.this.mapStartX >> 4)
                     + "/"
                     + (this.z + VillageMapInfo.this.mapStartZ >> 4)
                     + ", error at "
                     + var2.x
                     + "/"
                     + var2.z
