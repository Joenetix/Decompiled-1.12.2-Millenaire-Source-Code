...
public class GoalBreedAnimals extends Goal {
   ...
   private Item[] getBreedingItems(Class animalClass) {
      if (animalClass == EntityCow.class || animalClass == EntitySheep.class) {
         return CEREALS;
      } else if (animalClass == EntityPig.class) {
         return CARROTS;
      } else {
         return animalClass == EntityChicken.class ? SEEDS : null;
      }
   }
   ...
   public boolean performAction(MillVillager villager) throws Exception {
      for (Class animalClass : this.getValidAnimalClasses(villager)) {
         for (Entity ent : WorldUtilities.getEntitiesWithinAABB(villager.world, animalClass, villager.getPos(), 4, 2)) {
            if (!ent.isDead) {
               EntityAnimal animal = (EntityAnimal)ent;
               Item[] breedingItems = this.getBreedingItems(animal.getClass());
               boolean available = false;
               ...
               if (available && !animal.isChild() && !animal.isInLove() && animal.getGrowingAge() == 0) {
