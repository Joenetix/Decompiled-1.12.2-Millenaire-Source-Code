...
public class ContainerTrade extends Container {
   ...
   public ContainerTrade(EntityPlayer player, Building building) {
      ...
      if (sellingGoods != null) {
         for (TradeGood g : sellingGoods) {
            ...
            this.addSlotToContainer(new ContainerTrade.TradeSlot(building, player, true, g, 8 + 18 * (slotnb - 13 * slotrow), 32 + slotrow * 18));
            slotnb++;
         }
      }
      ...
      if (buyingGoods != null) {
         for (TradeGood g : buyingGoods) {
            int slotrow = slotnb / 13;
            this.addSlotToContainer(new ContainerTrade.TradeSlot(building, player, false, g, 8 + 18 * (slotnb - 13 * slotrow), 86 + slotrow * 18));
            slotnb++;
         }
      }
      ...
      for (int l = 0; l < 3; l++) {
         for (int k1 = 0; k1 < 9; k1++) {
            this.addSlotToContainer(new Slot(player.inventory, k1 + l * 9 + 9, 8 + k1 * 18 + 36, 103 + l * 18 + 37));
         }
      }

      for (int i1 = 0; i1 < 9; i1++) {
         this.addSlotToContainer(new Slot(player.inventory, i1, 8 + i1 * 18 + 36, 198));
      }
   ...
   public ContainerTrade(EntityPlayer player, MillVillager merchant) {
      ...
      if (sellingGoods != null) {
         for (TradeGood g : sellingGoods) {
            int slotrow = slotnb / 13;
            this.addSlotToContainer(new ContainerTrade.MerchantSlot(merchant, player, g, 8 + 18 * (slotnb - 13 * slotrow), 32 + slotrow * 18));
            slotnb++;
         }
      }
   ...
   public ItemStack slotClick(int slotId, int dragType, ClickType clickTypeIn, EntityPlayer player) {
      ...
      if (slotId >= 0 && slotId < this.inventorySlots.size()) {
         ...
         if (slot != null && slot instanceof ContainerTrade.TradeSlot) {
            ...
            if (tslot.isProblem() == null) {
               int playerMoney = MillCommonUtilities.countMoney(player.inventory);
               if (tslot.sellingSlot) {
                  ...
                  MillCommonUtilities.changeMoney(player.inventory, -good.getCalculatedSellingPrice(this.building, player) * nbItems, player);
                  ...
                  WorldUtilities.getItemsFromChest(player.inventory, good.item.getItem(), good.item.meta, nbItems);
                  if (!profile.donationActivated) {
                     MillCommonUtilities.changeMoney(player.inventory, good.getCalculatedBuyingPrice(this.building, player) * nbItems, player);
                  }
         ...
         if (slot != null && slot instanceof ContainerTrade.MerchantSlot) {
            ContainerTrade.MerchantSlot tslotx = (ContainerTrade.MerchantSlot)slot;
            good = tslotx.good;
            int nbItemsx = 1;
            if (dragType == 1) {
               nbItemsx = 64;
            ...
            if (tslotx.isProblem() == null) {
               int playerMoney = MillCommonUtilities.countMoney(player.inventory);
               if (playerMoney < good.getCalculatedSellingPrice(this.merchant) * nbItemsx) {
                  nbItemsx = MathHelper.floor(playerMoney / good.getCalculatedSellingPrice(this.merchant));
               }

               if (this.merchant.getHouse().countGoods(good.item) < nbItemsx) {
                  nbItemsx = this.merchant.getHouse().countGoods(good.item);
               }

               nbItemsx = MillCommonUtilities.putItemsInChest(player.inventory, good.item.getItem(), good.item.meta, nbItemsx);
               MillCommonUtilities.changeMoney(player.inventory, -good.getCalculatedSellingPrice(this.merchant) * nbItemsx, player);
               this.merchant.getHouse().takeGoods(good.item, nbItemsx);
               Mill.getMillWorld(player.world).getProfile(player).adjustLanguage(this.merchant.getCulture().key, nbItemsx);
            }
            ...
            if (!this.merchant.world.isRemote) {
               UserProfile profilex = this.merchant.mw.getProfile(player);
   ...
   public static class MerchantSlot extends Slot {
      ...
      public String isProblem() {
         if (this.merchant.getHouse().countGoods(this.good.item) < 1) {
            ...
            int playerMoney = MillCommonUtilities.countMoney(this.player.inventory);
   ...
   public static class TradeSlot extends Slot {
      ...
      public ItemStack getStack() {
         return this.sellingSlot
            ...
            : new ItemStack(
               this.good.item.getItem(),
               Math.max(Math.min(MillCommonUtilities.countChestItems(this.player.inventory, this.good.item.getItem(), this.good.item.meta), 99), 1),
               this.good.item.meta
            );
      }
      ...
      public String isProblem() {
         ...
         } else if (MillCommonUtilities.countChestItems(this.player.inventory, this.good.item.getItem(), this.good.item.meta) == 0) {
