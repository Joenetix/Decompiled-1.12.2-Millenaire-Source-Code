...
public class GuiQuest extends GuiText {
   ...
   public void buttonPagination() {
      ...
      if (this.type == 0) {
         if (this.firstStep) {
            if (this.showOk) {
               this.buttonList
                  .add(new GuiButton(1, xStart + this.getXSize() / 2 - 100, yStart + this.getYSize() - 40, 95, 20, LanguageUtilities.string("quest.refuse")));
               this.buttonList
                  .add(new GuiButton(0, xStart + this.getXSize() / 2 + 5, yStart + this.getYSize() - 40, 95, 20, LanguageUtilities.string("quest.accept")));
            } else {
               this.buttonList
                  .add(new GuiButton(1, xStart + this.getXSize() / 2 - 100, yStart + this.getYSize() - 40, 95, 20, LanguageUtilities.string("quest.refuse")));
               this.buttonList
                  .add(new GuiButton(2, xStart + this.getXSize() / 2 + 5, yStart + this.getYSize() - 40, 95, 20, LanguageUtilities.string("quest.close")));
            }
         } else if (this.showOk) {
            this.buttonList
               .add(new GuiButton(0, xStart + this.getXSize() / 2 - 100, yStart + this.getYSize() - 40, LanguageUtilities.string("quest.continue")));
         } else {
            this.buttonList
               .add(new GuiButton(2, xStart + this.getXSize() / 2 - 100, yStart + this.getYSize() - 40, LanguageUtilities.string("quest.close")));
         }
      } else {
         this.buttonList.add(new GuiButton(2, xStart + this.getXSize() / 2 - 100, yStart + this.getYSize() - 40, LanguageUtilities.string("quest.close")));
      }
   }
   ...
   private TextBook getData(int type, String baseText) {
      ...
      text.add(
         new TextLine(
            this.villager.getName() + ", " + this.villager.getNativeOccupationName() + game, "§1", new GuiText.GuiButtonReference(this.villager.vtype)
         )
      );
