...
public class GuiFirePit extends GuiContainer {
   ...
   public GuiFirePit(EntityPlayer player, TileEntityFirePit firePit) {
      super(new ContainerFirePit(player, firePit));
      this.player = player;
   ...
   protected void drawGuiContainerBackgroundLayer(float partialTicks, int mouseX, int mouseY) {
      ...
      if (this.firePit.getBurnTime() > 0) {
         ...
         this.drawTexturedModalRect(x + FIRE[0], y + FIRE[1] + 12 - burn, this.xSize, 12 - burn, 14, burn + 1);
      }
      ...
      for (int i = 0; i < 3; i++) {
         ...
         this.drawTexturedModalRect(x + arrowX, y + arrowY, this.xSize, arrowTexY, progress, arrowHeight);
      }
   }
